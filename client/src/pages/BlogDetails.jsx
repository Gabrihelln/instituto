...
import { PopularPost, PopularWriter } from "../components";
      ...
      <div className="w-full flex flex-col-reverse md:flex-row gap-2 gap-y-5 items-center">
        <div className="w-full md:w-1/2 flex flex-col gap-8">
          ...
          <div className="w-full flex items-center">
            <span className="flex-1 text-blue-600 font-semibold">
              {post?.cat}
            </span>
          ...
          <Link to={`/writer/${post?.user?._id}`} className="flex gap-3">
            <img src={post?.user?.image} alt={post?.user?.name} className="object-cover w-12 h-12 rounded-full" />
            <div className="">
        ...
        <img src={post?.img} alt={post?.title} className="w-full md:w-1/2 h-auto md:h-[360px] 2xl:h-[460px] rounded object-cover"/>
      </div>
